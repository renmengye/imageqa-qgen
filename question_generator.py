...
def questionGen(parseFilename, outputFilename):
    """Generates questions.
    """
    startTime = time.time()
    qCount = 0
    ...
    log.info('Time elapsed: {:d} seconds'.format(time.time() - startTime))
    log.info('Number of questions: {:d}'.format(qCount)

    log.info('Writing to output {}'.format(os.path.abspath(outputFilename)))
...
def stanfordParse(parserFolder, sentence):
...
def runSentence(parserFolder, sentence):
    ...
    """
    s=stanfordParse(parserFolder, sentence)
    s=s.split('\n')
    parser=TreeParser()
    gen=QuestionGenerator()
...
def parseArgs():
    ...
    parser.add_argument(
        '-sentence', help='Single sentence input')
